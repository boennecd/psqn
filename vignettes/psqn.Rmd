...
$\mat B_0$ and 
 ...
 2. find a step size $\alpha$ such that $\vec x_{k - 1} + \vec\alpha\vec p_k$ satisfy 
...
we have implemented starts with $\mat B_{k1},\dots,\mat B_{kn}$ and 
 ...
 2. finds a step size $\alpha$ such that $\vec x_{k - 1} + \vec\alpha\vec p_k$ satisfy 
 the [Wolfe conditions](https://en.wikipedia.org/wiki/Wolfe_conditions), 
...
$\mathcal O(n(p + q_{\text max})^2)$ where 
$q_{\text max} = \text{max}_{i = 1,\dots, n} q_i$ whereas the matrix-vector product 
in the BFGS method is $\mathcal O((p + \sum_{i = 1}^n q_i)^2)$. Further,
the rank-two BFGS updates are cheaper. However, we should keep in mind that 
the original BFGS method yields an approximation of $\mat B_k^{-1}$. Thus, 
we do not need to solve a linear system.
...
4. set $\vec\beta_{k + 1} = (\vec r_k^\top\vec r_k)^{-1}\vec r_{k + 1}^\top\vec r_{k + 1}$, 
