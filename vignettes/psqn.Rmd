...
by $\vec y_i$, and $\vec u$ which is a 
...
where $p_i(\vec u_i \mid \vec y_i;\vec\omega)$ is the conditional density 
...
\Expe\left(h(\vec U_i)\right) =
  \int h(\vec u) p_i(\vec u \mid \vec y_i;\widehat{\vec\omega})\der\vec u 
  \approx 
  \int h(\vec u) v_i(\vec u; \widehat{\vec\theta}_i)\der\vec u.
$$

The latter integral may be much easier to work with for a given function 
...
#   par the global and private parameter for this cluster. 
...
library(psqn)
r_psqn_func <- function(par, n_threads = 2L, cg_rel_eps = 1e-3, c1 = 1e-4, 
                        c2 = .9)
  psqn(par = par, fn = r_func, n_ele_func = n_clusters, 
       n_threads = n_threads, cg_rel_eps = cg_rel_eps, c1 = c1, c2 = c2)
...
tmp <- rnorm(idx_start)
all.equal(fn_one(tmp), fn_two(tmp))
fn <- fn_two
...
psqn_func <- function(par, n_threads = 2L, cg_rel_eps = 1e-4, c1 = 1e-4, 
                      c2 = .9, trace = 0L)
...
# use the function
r_psqn_func <- function(par, n_threads = 2L, cg_rel_eps = 1e-4, c1 = 1e-4, 
                        c2 = .9, trace = 0L)
  psqn(par = par, fn = r_func, n_ele_func = n_clusters, 
       n_threads = n_threads, cg_rel_eps = cg_rel_eps, c1 = c1, c2 = c2, 
