...
argument is added beacuse of the considerations above.
...
which does not have the same structure as the problem we have shown before. 
The problem we consider is:

$$
\begin{align*}
f(\vec x) &= \sum_{i = 1}^n 
  \left(\exp\left(\sum_{j = 1}^{L_i} x_{k_{ij}}\right) - \mu_i
  \right)^2, \qquad \vec x\in\mathbb R^K \\
\mathcal K_i &= \{k_{i1} , \dots, k_{iL_i}\} \subseteq \{1, \dots, K\}
\end{align*}
$$

Notice for some $j\neq i$ we may have that 
$\mathcal K_i \cap \mathcal K_j \neq \emptyset$. There is a class called 
`optimizer_generic` provided by the psqn package which can work with more 
general partially separable problems like the one above. 
...
We now simulate some data to work with this type of model:
...
R_func <- function(x){
  out <- vapply(dat, function(z) (exp(sum(x[z$indices])) - z$mu)^2, 0.)
  sum(out)
}
...
R_func_gr <- function(x){
  ...
  for(z in dat){
    exp_x_sum <- exp(sum(x[z$indices]))
    delta <- 2 * (exp_x_sum - z$mu) * exp_x_sum
    out[z$indices] <- out[z$indices] + delta
  }
  out
}
...
# TODO: delete
delta <- grad_generic_ex(noise, ptr = ptr, n_threads = 1L) - 
  grad_generic_ex(noise, ptr = ptr, n_threads = 2L)

# optimize and compare the result with one thread
psqn_func <- function(par, n_threads = 1L, c1 = 1e-4, c2 = .1, trace = 0L)
  optim_generic_ex(val = par, ptr = ptr, rel_eps = 1e-8, max_it = 1000L, 
...
opt_psqn$value - opt$value # (lower implies better solution)

# we can do the same with two threads
opt_psqn <- psqn_func(start, n_threads = 3L)
all.equal(opt_psqn$par, opt$par, tolerance = 1e-3)
all.equal(opt_psqn$value, opt$value)
opt_psqn$value - opt$value # (lower implies better solution)
```
