...
\end{align*}
$$
This is a special kind of a GLM with a Poison model with and log link. 
While there
are other ways to estimate this model, we will compare the BFGS implementation 
from `optim` with the psqn package.
...
dat <- replicate(
  n, {
    # sample the indices and make it more likely to sample some close together
    mid <- sample.int(K, 1L)
    n_other <- sample.int(5L, 1L) + 1L
    probs <- 1/(1 + (mid - seq_len(K)[-mid])^2)
    probs <- probs / sum(probs)
    other <- sample(seq_len(K)[-mid], n_other, prob = probs)
    indices <- sort(c(mid, other))
