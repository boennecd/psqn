...
$\lVert\vec r_k\rVert < \min (c, \lVert\nabla f(\vec x_{k -1})\rVert)\lVert\nabla f(\vec x_{k -1})\rVert$
where $c$ is a constant the user can set.
...
is implemented as an alternative to the BFGS. The user can set whether  
the SR1 updates should be used. The SR1 updates does not guarantee that the 
Hessian approximation is positive definite. Thus, the conjugate gradient 
method only proceeds if $\vec p_k^\top\mat A\vec p_k > 0$. That is, if the 
new direction is a descent direction.
