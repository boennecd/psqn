...
$\mathcal O(n(p + q_{\text max})^2)$ time. The conjugate gradient method
...
bench::mark(BFGS = optim_func(start_val), psqn = psqn_func(start_val), 
            check = FALSE)
```

We see a large reduction.
