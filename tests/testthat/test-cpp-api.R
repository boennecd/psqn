...
test_that("mixed logit model gives the same", {
  ...
  opt <- optim_mlogit(
    ...
    n_threads = 1L)
  opt_res <- list(par = c(0.647275645643599, 0.773166407590345, 0.657018688260653,
                          0.119620391547709, 0.855935190888554, -0.484320891862002, 0.412483447947671,
                          0.766965414435937, 0.422820993466102, 0.605160885730587, -0.783750641050074,
                          0.0961498666604934, -0.922989682244883, -0.483529637823472, 0.438265027742914,
                          -0.49328289152098, 0.0274560110134021, 0.426867821627715, 0.0283979684543824,
                          -0.180765758100669, -0.150514175218012), value = 25.121501447215,
                  info = 0L, counts = c(`function` = 21, gradient = 14, n_cg = 120
                  ), convergence = TRUE)
  expect_equal(opt[do_check], opt_res[do_check], tolerance = tol)
  opt <- optim_mlogit(
    val = val, ptr = optimizer, rel_eps = rel_eps, max_it = 100L,
